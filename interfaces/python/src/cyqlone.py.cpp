...
struct PythonCyqloneSolver {
    ...
    PythonCyqloneSolver(const PythonOCP &ocp,
                        cyqlone::qpalm::CyQPALMBackendSettings backend_settings,
                        cyqlone::qpalm::Settings qpalm_settings)
        : solver{cyqlone::qpalm::make_cyqpalm_backend<VL>(cyqlone::CyqloneStorage<>::build(ocp.ocp),
                                                          {}, backend_settings),
                 qpalm_settings} {}
};
...
void register_cyqlone_solver(nb::module_ &m) {
    ...
    solver //
        .def(
            "__init__",
            [](Solver &self, const CyqloneStorage<> &ocp, index_t p) {
                new (&self) Solver(Solver::build(ocp, p));
            },
            "ocp"_a, "p"_a)
        .def(
            "__init__",
            [](Solver &self, const PythonOCP &ocp, index_t p) {
                new (&self) Solver(Solver::build(CyqloneStorage<>::build(ocp.ocp), p));
            },
            "ocp"_a, "p"_a)
        ...
        .def(
            ...
            [](Solver &self, real_t S, np_batched_view<v, const real_t> Σ,
               const CyqloneStorage<> &ocp) {
                ...
                cyqlone::linalg::negate(ux); // right-hand side is the negative gradient
