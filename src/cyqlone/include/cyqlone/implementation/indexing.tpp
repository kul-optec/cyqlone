...
auto TricyqleSolver<VL, T, DefaultOrder>::sub_wrap_ceil_p(index_t a, index_t b) const -> index_t {
...
template <index_t VL, class T, StorageOrder DefaultOrder>
auto TricyqleSolver<VL, T, DefaultOrder>::add_wrap_ceil_p(index_t a, index_t b) const -> index_t {
    const index_t c = ceil_p();
    BATMAT_ASSUME(c > 0);
    BATMAT_ASSUME(a >= 0);
    BATMAT_ASSUME(b >= 0);
...
template <index_t VL, class T, StorageOrder DefaultOrder>
[[nodiscard]] index_t TricyqleSolver<VL, T, DefaultOrder>::ν2(index_t bi) const {
    BATMAT_ASSUME(bi > 0);
    auto ui = static_cast<std::make_unsigned_t<index_t>>(bi);
    return static_cast<index_t>(std::countr_zero(ui));
}
template <index_t VL, class T, StorageOrder DefaultOrder>
[[nodiscard]] index_t TricyqleSolver<VL, T, DefaultOrder>::ν2p(index_t bi) const {
    BATMAT_ASSUME(bi >= 0);
