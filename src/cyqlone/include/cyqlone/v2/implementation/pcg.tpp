...
auto CyqloneSolver<VL, T, DefaultOrder>::mul_Mv(batch_view<> p, mut_batch_view<> Ap,
                                                batch_view<default_order> L,
                                                batch_view<default_order> K) const -> value_type {
    // Ap = M p = LLᵀ p + K p
    trmm(triu(L.transposed()), p, Ap);
    trmm(tril(L), Ap);
    syomv(tril(K), p, Ap);
...
void CyqloneSolver<VL, T, DefaultOrder>::solve_pcg(mut_batch_view<> λ,
    ...
    auto A = pcr_L.batch(0), K = cr_Y.batch(0);
    value_type rᵀz = [&] {
        GUANAQO_TRACE("PCG", 0);
        copy(λ, r);
        fill(value_type{}, λ);
        value_type rᵀz = mul_precond(r, z, Ap, A, K);
        copy(z, p);
        return rᵀz;
    }();
    ...
    for (index_t it = 0; it < pcg_max_iter; ++it) {
        ...
        value_type pᵀAp = mul_Mv(p, Ap, A, K);
        ...
        if (r2 < ε2)
            break;
        value_type rᵀz_new = mul_precond(r, z, Ap, A, K);
